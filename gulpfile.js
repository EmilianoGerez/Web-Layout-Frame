...
gulp.task('njk-docs', getTask('njk-docs-task'));
...
gulp.task('watch', function() {
  ...
  gulp.watch([path.docslayoutFiles, path.docstemplateFiles], ['njk-docs']);
  gulp.watch([path.sassFiles], ['sass']);
  gulp.watch([path.jsFiles], ['scripts']).on("change", browserSync.reload);
})

gulp.task('serve', ['njk', 'njk-docs', 'sass', 'js-lint', 'scripts', 'watch'], function() {
  browserSync.init({
    server: {
      baseDir: './app'
    }
